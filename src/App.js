...
const CATEGORIES = [
  ...
  { id: "expos", label: "Musées & Expos", icon: "🏛️" },
...
function ExpandableText({ text, maxLength = 120 }) {
  ...
  return (
    <div style={{ margin: "0 0 14px" }}>
      ...
      >
        {isLong && !expanded ? text.slice(0, maxLength).trim() + "..." : text}
      </p>
      {isLong && (
        <button
...
function RecoCard({ reco, onLike, onDelete, liked, isOwner, delay }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "18px 20px 16px" }}>
        ...
        {reco.description && (
          <ExpandableText text={reco.description} />
        )}
...
function AddModal({ onClose, onSubmit, username, userId, onSetUsername }) {
...
export default function App() {
  ...
  const handleDelete = async (reco) => {
    if (!window.confirm("Supprimer cette recommandation ?")) return;
    try {
      await deleteDoc(doc(db, "recos", reco.id));
    } catch (err) {
          ...
          >
            {filtered.map((reco, i) => (
              <RecoCard
                ...
                isOwner={reco.authorId === userId}
                onLike={() => handleLike(reco)}
                onDelete={() => handleDelete(reco)}
                delay={i * 60}
              />
            ))}
          </div>
